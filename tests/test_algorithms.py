...
class TestEDFScheduler:
    ...
    def test_edf_optimality_single_machine(self):
        ...
        assert all(task.meets_deadline() for task in scheduler.tasks)
...
class TestPriorityFirstScheduler:
    ...
    def test_priority_first_can_cause_starvation(self):
        ...
        high_priority_completion = max(
            task.completion_time for task in scheduler.tasks if task.priority == Priority.HIGH
        )
...
class TestDPEScheduler:
    ...
    def test_dpe_elevates_low_priority_when_pressure_high(self):
        ...
        # Low priority task with tight deadline should get elevated and execute early
        # (exact timing depends on simulation, but it should complete before missing deadline)
        assert tasks[1].meets_deadline()
...
class TestAlgorithmComparison:
    ...
    def test_algorithms_produce_different_schedules(self):
        ...
        spt.run()
        spt_order = [(t.id, t.start_time) for t in spt.tasks]

        # Run with EDF
        edf = EDF_Scheduler(tasks.copy(), num_machines=1)
        edf.run()
        edf_order = [(t.id, t.start_time) for t in edf.tasks]

        # Run with Priority-First
        priority = PriorityFirst_Scheduler(tasks.copy(), num_machines=1)
        priority.run()
        priority_order = [(t.id, t.start_time) for t in priority.tasks]
